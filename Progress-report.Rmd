...
## NEXT:
1. Diagnostics for Standard Errors (In progress — almost done!)

    - You’re testing/visualizing analytic, bootstrap, and clustered SEs.
...
4. Package Structure
    ...
    - Ensure all functions are documented with @title, @description, @param, @return.
    - Add examples to each function using @examples.
    - Check that all helper functions are exported via NAMESPACE.
    - Consider adding a NEWS.md file for versioning.
    - Ensure DESCRIPTION has all dependencies listed (e.g., ggplot2, dplyr, etc.).
    
5. Optional Enhancements

    - Add a helper to compute average pre-trend fit quality or diagnostics.
    - Consider adding unitwise forecasts as an exportable function (fit_unitwise_trend()). Should we make the unitwise forecast exportable? At the moment, you can only call fit_unitwise_trend() inside estimate_fat(). This would allow users to replicate forecasts for individual units, which can be useful for transparency and verification.
...
✅ 4. Best practice for future diagnostics

To keep diagnostics clean and reproducible:

    - Create one .R file per diagnostic task in dev/
    - Name them clearly: e.g. se_diagnostic.R, placebo_diagnostic.R, timing_test.R
    - Always start each script with devtools::load_all() and data(...)
    - Use consistent naming (results_...) and concise plots with ggplot2
...
This would:
    ...
    Let them replicate and verify one unit’s forecasts manually
...
📚 Now, for the heterogeneous case (beta_estimator = "unitwise"):

In the paper (Botosaru, Giacomini, Weidner, 2023), section 4.2 considers the case where each unit has its own coefficients — that is:
yi,τ+1(0)=∑k=0qck(i)(τ+1)k+β(i)xi,τ+1
yi,τ+1​(0)=k=0∑q​ck(i)​(τ+1)k+β(i)xi,τ+1​

This is equation (25) in the paper.

➡️ That means for each unit i, you're estimating the coefficients of a time polynomial of degree q = degree, plus the unit-specific covariate coefficients β^(i).

To estimate this, the paper assumes a regression model of the form:
yit=∑k=0qck(i)tk+β(i)xit+ϵit
yit​=k=0∑q​ck(i)​tk+β(i)xit​+ϵit​

This is essentially a multiple regression with time terms and covariates.
🚫 Why degree = 0 is problematic here

When degree = 0, this equation becomes:
yit=c0(i)+β(i)xit+ϵit
yit​=c0(i)​+β(i)xit​+ϵit​

Which looks fine, but here’s the problem:

    Without variation in time (i.e., no time trend), we cannot forecast future values of the outcome in the presence of time dynamics — which is exactly the motivation for the polynomial trend.
...
So:
    ...
    If you estimate only a level (no trend), you’re discarding what makes the unitwise forecasting method useful.
...
The paper assumes q ≥ 1 implicitly for all forecasting models involving trends.



#### Extension: control group (DFAT)
Problem:
In the baseline FAT estimator, the counterfactual outcomes yi,τ+h(0)yi,τ+h​(0) must be forecasted unbiasedly using only pre-treatment data. This requires strong assumptions, especially the absence of unobservable macro shocks after treatment.
✅ Solution with Control Group:

Let Di=1Di​=1 if unit ii is treated, and Di=0Di​=0 otherwise.
...
DFAT^h=1n1∑i:Di=1(yi,τ+h−y^i,τ+h(0))−1n0∑i:Di=0(yi,τ+h−y^i,τ+h(0))
DFAT
h​=n1​1​i:Di​=1∑​(yi,τ+h​−y^​i,τ+h​(0))−n0​1​i:Di​=0∑​(yi,τ+h​−y^​i,τ+h​(0))

This means:
FAT (treated) − FAT (control) = DFAT.

It removes time effects that impact both groups equally (e.g. common macro shocks).
💡 2. Intuition in Your Package Context
    ...
    In the control group extension, you compute two sets of forecasts:
        ...
        One for the control group — although not treated, they’re used to compute parallel (counterfactual) forecasts.
...
In helper-function (specify), include sdFAT asjustment as $ sdDFAT sdDFATh​=n1​sdFATtreated2​​+n0​sdFATcontrol2​​
​
...
✅ Should we create a separate estimate_dfat() function?
💡 Option 1: Keep everything inside estimate_fat() (your current setup)
...
💡 Option 2: Add a wrapper estimate_dfat() (or even make it the main entry point for DFAT)
...
➡️ Recommendation: Keep everything in estimate_fat() for now, as you’re still actively developing the package. Once the structure stabilizes, a separate estimate_dfat() wrapper might be helpful for clarity and defaults.
