...
$$
\widehat{y}_{i \tau + h}(0) = \sum_{k=0}^{q_i} \widehat{c}_{ik}^{(q_i, R_i)} (\tau + h)^k
$$
...
$$
\widehat{c}^{(q_i, R_i)}_i = \arg\min_{c \in \mathbb{R}^{q_i+1}} \sum_{t \in T_i} \left(y_{it} - \sum_{k=0}^{q_i} c_k t^k\right)^2
$$
...
To incorporate additional covariates $x_{it}$ (e.g., policy drivers, controls), assume:
...
$$
\widehat{y}_{i,\tau + h}(0) = x_{i,\tau + h}' \widehat{\beta} + \sum_{k=0}^{q_i} \widehat{c}_{ik} (\tau + h)^k
$$
...
### Use Case
Use this when some covariates (e.g. lagged outcomes, instruments, or exogenous controls) explain common dynamics.
...
$$
y_{it}(0) = \sum_{k=0}^{q_i} c_{ik} t^k + \varepsilon_{it}
$$
...
$$
DFAT_h := FAT_h^{treated} - FAT_h^{control}
$$
